...
-- Add column to root disk size on service_offering
ALTER TABLE `cloud`.`service_offering` ADD COLUMN `root_disk_size` bigint DEFAULT 0 AFTER `ram_size`;

-- Service offering with root disk size
...
CREATE VIEW `cloud`.`service_offering_view` AS
    SELECT
        ...
        `service_offering`.`root_disk_size` AS `root_disk_size`,
